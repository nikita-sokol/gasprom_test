import { getServiceNameRequestAction, getServiceNameSuccessAction, getServiceNameErrorAction } from "../store/serviceNameReduser";
import { getServiceContentRequestAction, getServiceContentSuccessAction, getServiceContentErrorAction } from "../store/serviceContentReduser";
    ...
    return dispatch => {
        if (!store.services.find(i => i.id == params.id)) {
            ...
            fetch(`http://localhost:7070/api/services/${params.id}`)
                ...
                .catch(error => {
                    dispatch(getServiceContentErrorAction());
                });
        }
    }
}
...
export const fetchServiseName = () => {
    return dispatch => {
        ...
        fetch('http://localhost:7070/api/services')
            ...
            .catch(error => {
                dispatch(getServiceNameErrorAction());
            });
    }
}
